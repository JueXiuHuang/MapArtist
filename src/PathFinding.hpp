...
class BotCraftFinder final
 ...
 public:
  pf::BlockType getBlockTypeImpl(const pf::Position &pos) const override {
  ...
  inline pf::Position getPlayerLocationImpl() const override {
    std::shared_ptr<Botcraft::LocalPlayer> local_player =
  ...
  inline int getMinYImpl() const override {
    return client->GetWorld()->GetMinY();
  }

  inline int getMaxYImpl() const override {
    return client->GetWorld()->GetHeight();
  }

  BotCraftFinder(Botcraft::BehaviourClient *_client)
      : TFinder<BotCraftFinder<TFinder, TEdge, TEstimate, TWeight>,
                pf::Position, TEdge, TEstimate, TWeight>(
